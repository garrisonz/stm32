...
static uint8_t ir_is_active(void)
{
    return GPIO_ReadPin(GPIOB, IR_PIN) == IR_ACTIVE_LEVEL;
}

int main(void)
{
    uint32_t count = 0;
    uint8_t ir_was_active = 0;

    RCC_EnableGPIOB();
    GPIO_ConfigOutputOpenDrain(GPIOB, SCL_PIN);
    GPIO_ConfigOutputOpenDrain(GPIOB, SDA_PIN);
    GPIO_ConfigInputPullUp(GPIOB, IR_PIN);
    scl(GPIO_PIN_SET);
    sda(GPIO_PIN_SET);
    ...
    while (1) {
        uint8_t ir_active = ir_is_active();

        if (ir_active && !ir_was_active) {
