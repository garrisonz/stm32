...
static volatile uint8_t timer_count_changed;
...
int main(void)
{
    ...
    oled_clear();
    timer_count = 0u;
    timer_count_changed = 0u;
    oled_show_count(displayed_count);
    tim2_init_1s();

    while (1) {
        if (timer_count_changed != 0u) {
            timer_count_changed = 0u;
            displayed_count = timer_count;
            oled_show_count(displayed_count);
        }
    }
}
